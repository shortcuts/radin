...
<repo-root>/.claude/.radin/
  ...
  state/
    ...
    prepared/
      <task-id>.json              # branch and worktree `prepare` chose for that task
    prompts/
      <task-id>-<kind>.md         # assembled sub-agent prompt, dispatched by path
  plans/
...
```bash
radin backlog <env|show|list|count|find|add|add-plan|append|meta|order|field|duplicates|plan-target|set-category|retitle|set-priority|set-deps|set-meta|remove|reconcile|epics|epic-add|epic-show|epic-move|epic-remove>   # dispatcher at ~/.claude/.radin/bin/radin, symlinked into ~/.local/bin
```
...
- `field <id-or-title> <TASK_FILE|TASK_BODY|EPIC_CONTEXT|TASK_ID|CATEGORY|PLAN_PATHS|SKILLS|SKILLS_DROPPED|ACCEPTANCE|FACTS|LOCATION>` — one Execution-prompt placeholder per call, rendered ready to substitute. One value per call rather than a `NAME<TAB>value` listing, because reading one value out of a listing is the model picking from output again. The resolve dies on zero or several matches, so the call's own exit code is the entry-still-exists check; `PLAN_PATHS`, `ACCEPTANCE`, `FACTS`, `LOCATION` and `EPIC_CONTEXT` exit 1 to say "no plan" / "no criteria" / "not in an epic", and `TASK_BODY` exits 1 on a missing or empty task file, and `SKILLS` is pre-filtered against the four classes a leaf sub-agent cannot run (asks the user, spawns its own agent, launches a workflow, recurses into radin), matched on the leading `/<name>` token only — `SKILLS_DROPPED` names what it removed
...
- `add <category> <title> [--skill <name>]...` — create task (body on stdin): slugifies title into id (dedupe on collision against the index's `id` fields), writes file, appends one line to index. It rejects a body line carrying one of the five index-line labels, naming the CLI call to use instead
...
`radin scope` (`lib/radin-scope.sh`) is the same offload for `radin-review`'s one input. It resolves a commit, a PR, a directory, the no-argument branch diff, or a range (`last commit`, `last <n> commits`, `<rev>..<rev>`) into `type`/`scope`/`command`/`passes` lines; `passes` names the ponytail skill(s) that scope type calls for, because mapping a type to a pass was the last thing the skill computed from this script's own output. `--in-scope` reads `path:line` citations on stdin and prints `in`/`out` per citation plus a final `dropped<TAB><n>`, which is the out-of-scope drop the skill used to do by eye against the diff. `--tasks` prints the completed task ids the scope's commits belong to, which is the scope-type branching (`git log` versus `gh pr view`) the review skill used to do itself; it joins through `radin state trace` rather than re-implementing the hash match. A `since <date>` argument resolves too: the phrase goes to git's approxidate, and the `since` prefix is what keeps a garbage argument — which approxidate also accepts — out of that branch. Format in [domain models](domain-models.md#review-scope-output-radin-scope).

`radin prompt <planning|execution|debug|factfind> <id>` (`lib/radin-prompt.sh`) assembles one sub-agent prompt: it reads the template in `lib/radin-prompt-<kind>.md`, drops each `<!-- if:NAME -->` block the task has no input for (no plan, no skills, no dependency commits, no acceptance criteria, and every category row but the entry's own), substitutes every `UPPERCASE` placeholder from `backlog field` and `state deps-check`, writes the prompt to `state/prompts/<id>-<kind>.md`, and prints `model<TAB><name>` and `prompt<TAB><path>`. The router dispatches `Read <path> and follow it.`, never the text: echoing a 6KB prompt back as a `Task` argument costs output tokens on every dispatch. The router used to read all four prompts at Phase 4 and substitute by hand; it now reads only the one it sends, already assembled, and the leaf gets no step its task cannot use. The execution prompt carries the task body inline — preceded by the epic's `DESCRIPTION.md` when the task lives in one — rather than the task file's path, so an execution sub-agent spends no read on its own task. One template file per kind rather than one shared file, so a `---` line inside a prompt body cannot end the extraction.

`radin state task-report <id> <the STATUS line>` is the same offload for everything that happens after a dispatch reports. It checks the tree first (a dirty tree fails the task whatever the line claimed), then runs `task-done` for a `SUCCESS` whose hash validates and the failure route otherwise, and prints one final `next<TAB>continue|debug|clarify FACT|clarify DECISION` line. The router routes on that line alone, instead of picking between three state verbs across four exit codes.
...
- `trace <id|commit|branch>` — the provenance lookup in all three directions from one verb: task id gives its commit and branch, commit hash gives the task that concluded it, branch name gives every task executed on it. Argument's shape decides by probing each store for a match, same convention as `radin scope`, so exit 2 lists every candidate reading when one token reads two ways and exit 1 means none did. Prints `task`/`commit`/`branch`/`worktree`/`plan`/`facts`/`status` TAB lines per matching task, `task` first so a repeated key is the block delimiter. A failed or blocked task never gets a completion line, so `state/prepared/<id>.json` answers its tree and the journal its status
- `completed-show <id>` — print that completion's `id`/`commit`/`title`/`branch`/`worktree`/`plan`/`ts` as TAB lines, exit 1 when not recorded. Absent key prints empty value, so a line written before provenance existed reads as unknown. `radin-review` reads the recorded `plan` as the spec
- `completed-list` — print `id<TAB>commit` per completion in file order (exit 1 when nothing is recorded), for the TUI's Done view
- `deps-check <id>` / `task-dir <id>` — `radin prompt`'s two reads: dependency commits (exit 1 naming the first unresolved one), and the task's worktree (`<repo-root>-<id>`) when it exists, else repo root. In worktree mode repo root is not the tree the sub-agent worked in, so every dirty check and stash goes through the same lookup
- `journal-tail [n]` — last n events from append-only `state/journal.jsonl`, written by every mutation above. Lets agent reconstruct what session already did after context compaction. Forensics only, never control flow
...
`radin-execute` and `radin-implement` share one body, `lib/radin-run.md`, which each `SKILL.md` tells the router to read in full. A skill file holds only its frontmatter and its **no-plan rule**, the one step that differs: `radin-execute` passes `task-next --plan-first`, so a task with no plan gets a planning sub-agent, and `radin-implement` passes no flag, so it goes straight to execution, whose execution prompt then implements from the entry text. One body, so a change to the loop is a one-file edit and the two skills cannot drift. Below, `radin-execute` names that shared body.
...
- `lib/radin-execute-recovery.md` — `recover` routing for tasks a dead session left `in_progress`, read only when `radin-state.sh stuck` exits 0. Most runs never load it.
- `lib/radin-execute-reporting.md` — the one thing `state report` cannot do: the duplicate id/title scan. Read at Phase 5.
...
Planning is unconditional for the same reason. The loop used to skip the
...
describes the code. Phase 4 plans one task at a time, immediately before that
