...
One agent still ships, opt-in at install time (`install.sh` asks, default no): `agents/radin-execute-background.md`, for the user who wants the backlog run in a background thread they visit themselves. It invokes `/radin-execute` and overrides exactly one step, so the skill stays the source of truth for every phase.
