...
```bash
radin state <steps-init|next-pending|task-next|start|stuck|triage|recover|recover-reject|set-status|remove|deps-check|completed-add|completed-get|completed-show|completed-list|trace|task-done|task-fail|task-diagnosis|dirty-recover|report|task-dir|prepare|dirty-check|stash|session-set|session-get|journal-tail>
```
...
- `triage <namespace-dir> <id>` — facts about what dead sub-agent left: `attempts`, `completed` hash, `worktree`, `branch`, `branch_commit` lines, `dirty_files` count. Prints facts, decides nothing — agent routes on them (see `radin-execute` Phase 1 step 3). Worktree path derived from task id (execution prompt pins it to `../<repo>-<id>`); branch read from `prepare`'s record, `branch<TAB>none` when nothing recorded it — a `branch: no` run lands on the user's own checkout, which no derivation from the id can see
...
- `completed-add <completed-file> <id> <hash> [title] [branch] [worktree] [plan]` — append completed task's commit, create file if absent. Title stored because completion deletes backlog entry, so nothing else can name task in final report; branch, worktree and plan are same story for provenance. `ts` generated here, never passed
- `completed-show <completed-file> <id>` — print that completion's `id`/`commit`/`title`/`branch`/`worktree`/`plan`/`ts` as TAB lines, exit 1 when not recorded. Absent key prints empty value, so line written before provenance existed reads as unknown. `radin-review` reads the recorded `plan` as the spec instead of guessing the path
...
- `trace <namespace-dir> <id|commit|branch>` — the provenance lookup in all three directions from one verb: task id gives its commit and branch, commit hash gives the task that concluded it, branch name gives every task executed on it. Argument's shape decides by probing each store for a match, same convention as `radin scope`, so exit 2 lists every candidate reading when one token reads two ways and exit 1 means none did. Prints `task`/`commit`/`branch`/`worktree`/`plan`/`facts`/`status` TAB lines per matching task, `task` first so a repeated key is the block delimiter. A failed or blocked task never gets a completion line, so `state/prepared/<id>.json` answers its tree and the journal its status — exactly the tasks someone most wants to trace
- `task-done <namespace-dir> <id> <hash>` — record success, drop backlog and steps entries, crash-safe order. Validates hash first: must be commit reachable from the branch `prepare` recorded (or `HEAD` when nothing recorded it) in the task's tree, else exit 3 and nothing written. Hash comes off a sub-agent's free-text `STATUS:` line, so nothing else checks it
...
- `journal-tail <namespace-dir> [n]` — last n events from append-only `state/journal.jsonl`, written by every mutation above. Lets agent reconstruct what session already did after context compaction. Forensics only, never control flow — `trace` reads it for reporting, which takes no branch
