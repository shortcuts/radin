...
one), and radin's lib scripts under `~/.claude/.radin/lib/`. It removes
only files radin can prove it shipped, by exact name — never a wildcard
...
repo. That backlog is the user's data, not something to delete on their
behalf.
