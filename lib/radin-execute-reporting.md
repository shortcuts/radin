...
4. Collect all commit hashes recorded this session, plus every `failed` and
   `blocked` entry in `BACKLOG_STEPS.json` with its `note`. Nothing verified
   these commits yet — Phase 6's `/radin-review` is where that happens. Say
   how many net-new backlog entries this session added, per
   `radin-backlog.sh count`.
